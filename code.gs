...
const TIMEZONE = "GMT+8"; // UPDATED: Set ke UTC+08:00 (WITA)
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0]) { 
        ...
        if (rawTgl instanceof Date) {
          ...
          tglLahir = Utilities.formatDate(rawTgl, 'GMT+8', 'yyyy-MM-dd');
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][1] == nisn) {
        ...
        let tglLahir = '';
        if (data[i][3]) {
          tglLahir = Utilities.formatDate(new Date(data[i][3]), 'GMT+8', 'yyyy-MM-dd');
        }
...
function scanAbsensi(nisn, scannerRole, scannerKelas) {
  try {
    ...
    const nowTime = Utilities.formatDate(new Date(), 'GMT+8', 'HH:mm');
    
    // 1. AMBIL KONFIGURASI
    const configResult = getAppConfig();
    const config = configResult.success ? configResult.data : {
      jam_masuk_akhir: '07:30',
      jam_pulang_senin_kamis_mulai: '14:00',
      jam_pulang_senin_kamis_akhir: '17:00',
      jam_pulang_jumat_mulai: '11:30',
      jam_pulang_jumat_akhir: '17:00',
      jam_pulang_sabtu_mulai: '12:00',
      jam_pulang_sabtu_akhir: '17:00'
    };
    ...
    if (liburSheet) {
      ...
      for (let i = 1; i < liburData.length; i++) {
        if (liburData[i][0]) {
          let tglLibur = Utilities.formatDate(new Date(liburData[i][0]), 'GMT+8', 'yyyy-MM-dd');
          if (tglLibur === today) {
    ...
    for (let i = 1; i < absensiData.length; i++) {
      ...
      const rowDateStr = Utilities.formatDate(new Date(rowDateCell), 'GMT+8', 'yyyy-MM-dd');
      ...
      if (rowDateStr === today && rowNisn === scannedNisn) {
        ...
        if (absensiData[i][5]) { 
          ...
          if (nowTime > config.jam_pulang_senin_kamis_akhir) {
             return { 
               success: false, 
               message: `Gagal! Batas waktu pulang (${config.jam_pulang_senin_kamis_akhir}) sudah lewat.` 
             };
          }
          ...
          let jamDatangStr = (jamDatangRaw instanceof Date) ? 
              Utilities.formatDate(jamDatangRaw, 'GMT+8', 'HH:mm') : 
          ...
          const jamMulaiPulang =
            day === 5 ? config.jam_pulang_jumat_mulai :
            day === 6 ? config.jam_pulang_sabtu_mulai :
            config.jam_pulang_senin_kamis_mulai;

          if (nowTime < jamMulaiPulang) {
            ketBaru = ketSaatIni + ' & Pulang Cepat';
            pesanPulang = 'Absen Pulang (Pulang Cepat)';
          }

          const jamPulang = Utilities.formatDate(new Date(), 'GMT+8', 'HH:mm:ss');
    ...
    const day = new Date().getDay();
    const jamAkhir =
      day === 5 ? config.jam_pulang_jumat_akhir :
      day === 6 ? config.jam_pulang_sabtu_akhir :
      config.jam_pulang_senin_kamis_akhir;

    if (nowTime > jamAkhir) {
      return {
        success: false,
        message: 'Absensi Ditutup! Sudah melewati jam operasional.'
      };
    }
   ...
   const jamDatang = Utilities.formatDate(new Date(),'GMT+8','HH:mm:ss');
...
function getAbsensiToday(nisn) {
  try {
    ...
    if (liburSheet) {
      const liburData = liburSheet.getDataRange().getValues();
      for (let i = 1; i < liburData.length; i++) {
        let tgl = Utilities.formatDate(new Date(liburData[i][0]), 'GMT+8', 'yyyy-MM-dd');
        if (tgl === todayStr) {
          isLibur = true;
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (rowDateStr === todayStr && rowNisn === searchNisn) {
        ...
        if (jamDatang instanceof Date) {
          jamDatang = Utilities.formatDate(jamDatang, 'GMT+8', 'HH:mm:ss');
        }
        
        // Format Jam Pulang
        let jamPulang = data[i][5];
        if (jamPulang instanceof Date) {
          jamPulang = Utilities.formatDate(jamPulang, 'GMT+8', 'HH:mm:ss');
        } else if (!jamPulang) {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0]) {
        ...
        let tanggalStr = Utilities.formatDate(rawDate, 'GMT+8', 'yyyy-MM-dd');

        // Format Jam
        let jamDatangStr = data[i][4];
        if (data[i][4] instanceof Date) {
             jamDatangStr = Utilities.formatDate(data[i][4], 'GMT+8', 'HH:mm:ss');
        }

        let jamPulangStr = data[i][5];
        if (data[i][5] && data[i][5] instanceof Date) {
             jamPulangStr = Utilities.formatDate(data[i][5], 'GMT+8', 'HH:mm:ss');
...
function setupInitialData() {
  try {
    ...
    if (!configSheet) {
      ...
      configSheet.appendRow(['jam_pulang_senin_kamis_mulai', '15:00', 'Waktu absen pulang dibuka']);
      configSheet.appendRow(['jam_pulang_senin_kamis_akhir', '17:00', 'Batas akhir absen pulang']);
      configSheet.appendRow(['jam_pulang_jumat_mulai', '15:00', 'Waktu absen pulang dibuka']);
      configSheet.appendRow(['jam_pulang_jumat_akhir', '17:00', 'Batas akhir absen pulang']);
      configSheet.appendRow(['jam_pulang_sabtu_mulai', '15:00', 'Waktu absen pulang dibuka']);
      configSheet.appendRow(['jam_pulang_sabtu_akhir', '17:00', 'Batas akhir absen pulang']);
    }
...
function getHariLibur() {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0]) {
        let tgl = Utilities.formatDate(new Date(data[i][0]), 'GMT+8', 'yyyy-MM-dd');
        list.push({
          tanggal: tgl,
          keterangan: data[i][1]
        });
...
function getMonitoringRealtime(filterKelas = null) {
  try {
    ...
    for (let i = 1; i < dataAbsensi.length; i++) {
      ...
      let tgl = Utilities.formatDate(new Date(rowDate), 'GMT+8', 'yyyy-MM-dd');
    ...
    for (let i = 1; i < dataSiswa.length; i++) {
      ...
      if (statusInfo) {
        ...
        if (statusInfo.jamDatang instanceof Date) {
            jamDatang = Utilities.formatDate(statusInfo.jamDatang, 'GMT+8', 'HH:mm');
        } else if (statusInfo.jamDatang) jamDatang = String(statusInfo.jamDatang);

        if (statusInfo.jamPulang instanceof Date) {
            jamPulang = Utilities.formatDate(statusInfo.jamPulang, 'GMT+8', 'HH:mm');
...
function updateAbsensiStatus(token, nisn, nama, kelas, newStatus) {
  try {
    ...
    if (found) {
      ...
      let jamDatang = '-';
      if (newStatus === 'Hadir') {
        jamDatang = Utilities.formatDate(new Date(), 'GMT+8', 'HH:mm:ss');
      }
...
function getExportData(type, filters) {
  ...
  if (type === 'laporan_absensi') {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      let rawDate = new Date(data[i][0]);
      let tanggalStr = Utilities.formatDate(rawDate, 'GMT+8', 'dd-MM-yyyy');
      let dateForFilter = Utilities.formatDate(rawDate, 'GMT+8', 'yyyy-MM-dd');
      ...
      if (match) {
        let jamDatang = data[i][4];
        if (data[i][4] instanceof Date) {
             jamDatang = Utilities.formatDate(data[i][4], 'GMT+8', 'HH:mm:ss');
        }
        let jamPulang = data[i][5];
        if (data[i][5] && data[i][5] instanceof Date) {
             jamPulang = Utilities.formatDate(data[i][5], 'GMT+8', 'HH:mm:ss');
        }
...
function generateExcel(type, filters) {
  try {
    // 1. SETUP FILE & SHEET
    var timestamp = Utilities.formatDate(new Date(), 'GMT+8', 'dd-MM-yyyy HHmm');
    var fileName = "";
    var headers = [];
...
function getAppConfig() {
  try {
    ...
    let config = {
      ...
      jam_pulang_senin_kamis_mulai: '14:00',
      jam_pulang_senin_kamis_akhir: '14:30',
      jam_pulang_jumat_mulai: '11:30',
      jam_pulang_jumat_akhir: '12:00',
      jam_pulang_sabtu_mulai: '12:00',
      jam_pulang_sabtu_akhir: '12:30'
    };
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (config.hasOwnProperty(key)) {
          ...
          if (val instanceof Date) {
            config[key] = Utilities.formatDate(val, 'GMT+8', 'HH:mm');
          } else {
...
function saveAppConfig(newConfig) {
  try {
    ...
    updateRow('jam_pulang_senin_kamis_mulai', newConfig.jam_pulang_senin_kamis_mulai);
    updateRow('jam_pulang_senin_kamis_akhir', newConfig.jam_pulang_senin_kamis_akhir);
    updateRow('jam_pulang_jumat_mulai', newConfig.jam_pulang_jumat_mulai);
    updateRow('jam_pulang_jumat_akhir', newConfig.jam_pulang_jumat_akhir);
    updateRow('jam_pulang_sabtu_mulai', newConfig.jam_pulang_sabtu_mulai);
    updateRow('jam_pulang_sabtu_akhir', newConfig.jam_pulang_sabtu_akhir);
